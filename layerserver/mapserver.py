...
class MapserverLayer(WMSProxy):
    ...
    def get_wms_buffer_enabled(self):
        return self.style_ob.get_shapetype() == 'circle'

    def get_wms_buffer_size(self):
        size = self.style_ob.max_size()
        if not size:
            size = 64
...
class StyleLayer(object):
    ...
    def render_rules(self):
        ...
        for x in self.layer.rules.all().order_by(F('order').asc(nulls_last=False)):
            ...
            r = self.render_style(x, str(x), expression)
            rules.append(r)
        return rules
    ...
    def write(self, request=None):
        ...
        styles.append(self.render_style(self.layer, self.layer.title or self.layer.name))
        return "\n".join(styles)

    def max_size(self, request=None):
        sizes = []
        if self.layer.shape_radius and self.isfloat(self.layer.shape_radius):
