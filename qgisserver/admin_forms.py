...
class ServiceChangeForm(TileCacheChangeFormMixin, forms.ModelForm):
    ...
    def clean(self):
        ...
        if tilecache_minzoom_level > tilecache_maxzoom_level:
            self.add_error('tilecache_minzoom_level', 'This camp must be lower or equal than Maximum zoom level')
            self.add_error('tilecache_maxzoom_level', 'This camp must be bigger or equal than Minimum zoom leve')

        if tilecache_minZoom_level_chached > tilecache_maxZoom_level_chached:
            self.add_error('tilecache_minZoom_level_chached', 'This camp must be lower or equal than Max zoom level cached')
            self.add_error('tilecache_maxZoom_level_chached', 'This camp must be bigger or equal than Min zoom level cached')
